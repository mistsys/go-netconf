...
// NewSessionFromSSHClient creates a new NETCONF session using an existing ssh client
func NewSessionFromSSHClient(client *ssh.Client) (*Session, error) {
	t := &TransportSSH{}
	t.sshClient = client
...
func DialSSHTimeout(target string, config *ssh.ClientConfig, timeout time.Duration) (*Session, error) {
	...
	t, err := connToTransport(conn, conn.RemoteAddr().String(), config)
	if err != nil {
		t.Close()
		return nil, err
	}
